...
class GPT(nn.Module):
    ...
    def forward_backbone(self, input_ids: Tensor) -> Tensor:
        """Run the backbone (embedding + blocks + norm) and return hidden states."""
        ...
        skips: list[Tensor] = []
        ve_cache: dict = {}
        # Build layer schedule: [0,1,...,N-1] with recur layers replayed
        schedule: list[int] = []
        ...
        # Track which layers have had their first-pass for skip logic
        seen: set[int] = set()
        for li in schedule:
